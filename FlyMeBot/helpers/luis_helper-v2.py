...
#_____________________V2___________________
#_________ (trial) use of datetime with consideration to daterange ___
...
#                departure_date_entities = recognizer_result.entities.get("Departure_Date", [])
#                departure_date_entities = recognizer_result.entities.get("DepartureDate", [])
...
#                arrival_date_entities = recognizer_result.entities.get("ArrivalDate", [])
                
                date_entities_type = recognizer_result.entities.get("type", [])
                print ("date_entities_type ", date_entities_type)

                if date_entities_type == "daterange":
                    departure_date_entities = recognizer_result.entities.get("daterange", [])
                    if departure_date_entities:
                        start = departure_date_entities[0]["start"]
                        print("start ", start)
                        if start:
                            #datetime = timex[0].split("T")[0]
                            result.departure_date = start
                        else:
                            result.departure_date = None 
                        print("Departure Date ", result.departure_date)
                
                        end = departure_date_entities[0]["end"]
                        print("end ", end)
                        if end:
                            #datetime = timex[0].split("T")[0]
                            result.departure_date = end
                        else:
                            result.arrival_date = None 
                        print("Arrival Date ", result.arrival_date)

                elif date_entities_type == "datetime":
                    ...
                    if departure_date_entities[1]:
                        timex = departure_date_entities[1]["timex"]
                        if timex:
                            datetime = timex[0].split("T")[0]
                            result.arrival_date = datetime
                    else:
                            result.arrival_date = None
                    print("Arrival Date ", result.arrival_date)
