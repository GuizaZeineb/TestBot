...
class MainDialog(ComponentDialog):
    def __init__(   
        self, luis_recognizer: FlightBookingRecognizer, booking_dialog: BookingDialog, telemetry_client: BotTelemetryClient = None,
    ):
...
#        self.add_dialog(TextPrompt(TextPrompt.__name__))
    ...
    async def final_step(self, step_context: WaterfallStepContext) -> DialogTurnResult:
        ...
        if step_context.result is not None:
            ...
            msg_txt = (
                    f"I have you booked to {result.destination} from {result.origin} on {result.departure_date}"
                    f" returning back on {result.arrival_date} with a maximum budget of {result.budget}"
            )
...
#    @staticmethod
#    async def _show_warning_for_unsupported_budget(
#        context: TurnContext, luis_result: BookingDetails
#    ) -> None:
#        if int(luis_result.budget) ==0 :
#            message_text = (
#                f"Sorry but a null budget is not supported:"
#                f" {', '.join(luis_result.budget)}"
#            )
#            message = MessageFactory.text(
#                message_text, message_text, InputHints.ignoring_input
#            )
