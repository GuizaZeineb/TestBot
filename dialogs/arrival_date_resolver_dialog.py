...
class ArrivalDateResolverDialog(CancelAndHelpDialog):
    
    def __init__(self, 
                dialog_id: str = None, 
                telemetry_client: BotTelemetryClient = NullTelemetryClient(),):

        super(ArrivalDateResolverDialog, self).__init__(
            dialog_id or ArrivalDateResolverDialog.__name__, telemetry_client
        )
        self.telemetry_client = telemetry_client

        date_time_prompt = DateTimePrompt(
            DateTimePrompt.__name__, ArrivalDateResolverDialog.datetime_prompt_validator
        )
    ...
    async def initial_step(
        ...
        # timex = booking_details.arrival_date

        timex = step_context.options

        prompt_msg_text = "On what date would you like to return back from travel?"
        ...
        # We have a Date we just need to check it is greater than departure date.
